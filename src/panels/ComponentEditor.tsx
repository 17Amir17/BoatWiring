      ...
      >
        ...
        <div className="flex flex-1 min-h-0">
          <div className="flex-1 p-3 overflow-auto">
            <div
              ref={imgBoxRef}
              className="relative bg-panel-hover rounded border border-panel-border mx-auto"
            ...
            >
              ...
              {draft.ports.map((p, i) => (
                ...
                >
                  ...
                  <div className="absolute -top-3 left-3 text-[9px] text-slate-200 whitespace-nowrap">
                    {p.label} <button className="text-red-400 ml-1" onClick={() => removePort(i)}>×</button>
                  </div>
                </div>
              ))}
            </div>
              ...
              <div className="space-y-1">
                {draft.ports.map((p, i) => (
                  <div key={p.id} className="grid grid-cols-[2fr_2fr_2fr_3fr] gap-1 text-xs">
                    <input
                      className="bg-panel-hover border border-panel-border rounded px-1"
          ...
          <div className="w-[300px] border-l border-panel-border p-3 overflow-auto text-xs space-y-2">
            <Field label="ID">
              <input
                className="w-full bg-panel-hover border border-panel-border rounded px-1"
            ...
            <Field label="Width / Height">
              <div className="flex gap-1">
                <input className="w-full bg-panel-hover border border-panel-border rounded px-1"
            ...
            <div>
              ...
              {Object.entries(draft.specs).map(([k, v]) => (
                <div key={k} className="flex items-center gap-1 mb-1">
                  <input className="w-1/3 bg-panel-hover border border-panel-border rounded px-1"
                    value={k}
                    onChange={(e) => {
                      setDraft((d) => {
                        const next = { ...d.specs };
                  ...
                  <input className="flex-1 bg-panel-hover border border-panel-border rounded px-1"
              ...
              <button
                className="text-[10px] px-2 py-1 rounded bg-panel-hover hover:bg-slate-700/40"
        ...
        <div className="flex items-center justify-end gap-2 px-3 h-10 border-t border-panel-border">
          <button className="text-xs px-3 py-1 rounded bg-panel-hover" onClick={onClose}>Cancel</button>
          <button
            className="text-xs px-3 py-1 rounded bg-emerald-700/60 hover:bg-emerald-700/80"
